...
function getActiveIndexes(s, e) {
  ...
  if (selected.length) {
    return selected.filter(i => i >= s && i <= e);
  }
...
function renderBlockStatsLine() {
  ...
  blockStatsEl.textContent =
    `選択中：${label}｜対象 ${indexes.length}語`;
}
...
const selectAllBlocksBtn =
  document.getElementById("selectAllBlocks");

const clearBlocksBtn =
  document.getElementById("clearBlocks");
...
clearBlocksBtn?.addEventListener("click", () => {
  ...
  const remain = active.filter(i => {
    return !askedSet.has(words[i].en);
  });
  ...
  if (!hasAnyWeak) {
    return pickIndex_random(s, e);
  }

  arr.sort((a, b) => {
    return (b.manual - a.manual) || (b.p - a.p);
  });
  ...
  if (fromGoimon) {
    ...
    if (routeText) {
      routeText.textContent =
        `ちえを伸ばす ${Math.floor(goimonCount)}問セット`;
    }
